...
#include "FSBuildableCircuitSwitchOperator.h"
...
AFGBuildable* UFSBuildableCircuitSwitchOperator::CreateCopy(const FSTransformOperator& TransformOperator)
{
	...
	AFGBuildableCircuitSwitch* SourceBuildableCircuitSwitch = nullptr;

	if (Source.Buildable) {
		Buildable = BuildableSubsystem->BeginSpawnBuildable(Source.Buildable->GetClass(), Transform);
		SourceBuildableCircuitSwitch = Cast<AFGBuildableCircuitSwitch>(Source.Buildable);
	}

	AFGBuildableCircuitSwitch* BuildableCircuitSwitch = Cast<AFGBuildableCircuitSwitch>(Buildable);

	BuildableCircuitSwitch->SetSwitchOn(SourceBuildableCircuitSwitch->IsSwitchOn());
