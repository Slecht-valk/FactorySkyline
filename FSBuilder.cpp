...
void AFSBuilder::Load(UFSDesign* DesignParam, bool FullPreview)
{
	...
	*/
	...
	for (TWeakObjectPtr<AFGBuildable> BuildablePtr : DesignParam->BuildableSet) if (BuildablePtr.Get()) {
		...
		if (Buildable == AnchorBuildable || Dist < 2500.0f || FullPreview) {
...
bool AFSBuilder::Build(FSRepeat* Repeat)
{
	...
	// TODO REWORK THIS
	//if (!this->Design->Anchor) return false;
...
FTransform AFSBuilder::GetFixedSourceTransform()
{
	...
	// TODO REWORK THIS
	/*
	Transform = this->Design->Anchor->GetTransform();
...
void UFSSyncBuild::DoWork()
{
	...
	if (Step == 0) {
		...
		AFSkyline* FSkyline = AFSkyline::Get(this);
		FSkyline->Builder->SyncBuild->StartDoWorkTimer(1.0f);
